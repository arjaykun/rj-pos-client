...
const Orders = ({ getOrders, orders, loading, change_url }) => {
  ...
  useEffect(() => {
    getOrders(
      `/orders?page=${page}&limit=${limit}&sort_by=${sort_by}&asc=${asc}&search_by=${search_by}&search=${search}&start=${start}&end=${end}`
    );
  ...
  return (
    <div className="px-2">
      ...
      <div className="p-2 px-2 mt-5 mb-2 flex justify-between items-center border-b border-t border-red-900">
        ...
        <tbody>
          {orders.orders.length > 0 ? (
            orders.orders.map((order, index) => (
              ...
              >
                <td className="py-2">
                  {moment(order.createdAt).format("MM/DD/YY hh:mm A")}
                </td>
                <td className="py-2">{order.order_id}</td>
                <td className="py-2">&#8369; {order.total}</td>
                <td className="py-2">
                  <Link
                    to={{ pathname: `/admin/orders/${order._id}`, data: order }}
                  >
                    <FaChevronCircleRight />
                  </Link>
                </td>
            ...
            <tr>
              <td
                colSpan="4"
                className="text-center text-4xl bg-gray-200 py-5 font-bold"
              >
                No Orders
              </td>
            </tr>

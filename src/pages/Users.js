...
const Users = ({
  ...
  useEffect(() => {
    getUsers(
      `/users?page=${page}&limit=${limit}&search_by=${search_by}&search=${search}&sort_by=${sort_by}&asc=${asc}`
    );
  ...
  const hideModal = () => {
    setShowModal(false);
    if (messages.error) clear_message();
  };
  ...
  const hideMessage = () => {
    if (messages.message) clear_message();
  };
  ...
  return (
    <div className="px-2">
      ...
      <div>
        <div className="py-2 px-2 mt-5 mb-2 flex justify-between items-center border-b border-t border-red-900">
      ...
      {messages.message ? <AlertMessage messages={messages} /> : null}
...
export default connect(
  mapStateToProps,
  { getUsers, change_url, clear_message, deleteUser }
)(Users);
